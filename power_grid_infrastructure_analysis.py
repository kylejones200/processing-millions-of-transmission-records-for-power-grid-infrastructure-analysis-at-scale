...
def generate_synthetic_grid_data(n_lines: int = 50000) -> pd.DataFrame:
    ...
    data = {
        ...
        "VOLTAGE": np.random.choice(
            [v[0] for v in voltage_classes], n_lines, p=[0.02, 0.08, 0.35, 0.40, 0.15]
        ),
        "VOLT_CLASS": "",
...
def plot_critical_corridors(service: TransmissionLinesService, plot: bool = False):
    ...
    if plot:
        ...
        plt.scatter(
            ...
            corridors["parallel_lines"],
            s=corridors["criticality"] * 10,
            alpha=0.6,
            c=corridors["criticality"],
            cmap="Reds",
            edgecolors="black",
            linewidth=0.5,
        )
...
def main():
    ...
    logger.info(
        f"In service:       {stats['in_service']:>10,} ({stats['in_service'] / stats['total_lines'] * 100:.1f}%)"
    )
